...
def run_dynamics_numba(indptr, indices, strategies, num_players, alpha, c, max_iterations):
    """
    Versione JIT-compilata della dinamica di Best Response.
    Mantiene la logica sequenziale (asincrona) ma gira a velocità C.
    """
    for iteration in range(max_iterations):
        changes_count = 0
        for player_id in range(num_players):
            # Check coverage (loop manuale ottimizzato per Numba)
...
class BestResponseDynamicsSparse:
    ...
    def run(self):
        print(f"Inizio simulazione dinamica su {self.num_players} nodi (JIT Optimized)...")
        start_time = time.time()

        # Preparazione dati per Numba (array NumPy sottostanti alla matrice sparsa)
        ...
        # Nota: La prima esecuzione includerà il tempo di compilazione (piccolo overhead iniziale)
        ...
        if used_iterations < max_iter:
            print(f"Convergenza raggiunta all'iterazione {used_iterations}.")
        else:
            print(f"Stop: Massimo iterazioni ({max_iter}) raggiunto.")
